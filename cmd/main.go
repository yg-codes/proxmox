...
import (
	...
	"github.com/yg-codes/proxmox/pkg/bulk"
	"github.com/yg-codes/proxmox/pkg/config"
	"github.com/yg-codes/proxmox/pkg/container"
	"github.com/yg-codes/proxmox/pkg/network"
	"github.com/yg-codes/proxmox/pkg/node"
	"github.com/yg-codes/proxmox/pkg/protection"
	"github.com/yg-codes/proxmox/pkg/resource"
	"github.com/yg-codes/proxmox/pkg/snapshot"
	"github.com/yg-codes/proxmox/pkg/storage"
	"github.com/yg-codes/proxmox/pkg/task"
...
var (
	...
	// VM node filter (persistent across all vm subcommands)
	vmNodeFlag   string
	vmStatusFlag string
	listAllFlag  bool
)
...
func init() {
	// VM command persistent flags (inherited by all subcommands)
	vmCmd.PersistentFlags().StringVar(&vmNodeFlag, "node", "", "Filter to VMs on a specific node (e.g. fsx-dev-pve22)")

	// Global flags
	rootCmd.SetVersionTemplate(fmt.Sprintf("pve {{.Version}} (commit: %s, built: %s)\n", commit, date))
...
func runListVMsCommand(cmd *cobra.Command, args []string) error {
	...
	if vmStatusFlag != "" {
		...
		for _, v := range vms {
			...
			default:
				return fmt.Errorf("invalid status filter %q: use running or stopped", vmStatusFlag)
			}
		}
		vms = filtered
	}
